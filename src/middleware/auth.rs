use actix_web::{
    body::BoxBody, dev::{ServiceRequest, ServiceResponse}, error::ErrorUnauthorized, middleware::Next, web, HttpMessage, Error,
};
...
pub async fn verify_jwt(
    ...
    let auth_header = req.headers().get("Authorization").ok_or_else( || {
        ErrorUnauthorized(json!({
            "status": "error",
            "message": "Unauthorization header is missing"
        }))
    })?;
    ...
    let auth_str = auth_header.to_str().map_err(|_|{
        ErrorUnauthorized(json!({
            "status": "error",
            "message": "Unauthorization header is malformed!"
        }))
    })?;
